...
`host/green.py` 是运行在主机上的命令行客户端，通过 TCP 与手机端的
...
python3 host/green.py ps [filter]
python3 host/green.py attach -p <pid> -l script.js      # 脚本文件（主机路径）
python3 host/green.py attach -f com.example.app \
    -c "console.log('pid', Process.id)"
python3 host/green.py --host <device-ip> --port 27042 ps   # 免 adb forward
```
