...
green_gum_shadow_patch (gconstpointer address,
                        ...
                        gsize len)
{
  /* Payload variant: the target is unprivileged, so the shadow commit goes
   * through the root broker as a full-page image. */
  guint8 * page_addr = (guint8 *) (((guintptr) address) & ~4095ULL);
  gsize page_off = (guint8 *) address - page_addr;
  guint8 page[4096];

  if (len == 0 || page_off + len > 4096)
    return FALSE;

  if (green_gum_process_vm (FALSE, page, 4096, page_addr) != 4096)
    return FALSE;

  memcpy (page + page_off, bytes, len);
  return green_agent_broker_page_commit (GPOINTER_TO_SIZE (page_addr),
                                         page, 4096) == 0;
}
