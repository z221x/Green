...
- **硬性条件：设备 CPU 必须实现 FEAT_EPAN**（`ID_AA64MMFR1_EL1.EPAN` ≠ 0，ARMv8.7/v9.2+ 核；内核建议 5.13+）。shadow 执行视图是 execute-only 映射（`PAGE_EXECONLY` 编码，AP[2:1]=10 + UXN=0）：无 EPAN 时这类映射可被 EL1 无声读取（上游内核 2019 年因此回退过 exec-only 支持），KPM 初始化会检测并拒绝上线。
