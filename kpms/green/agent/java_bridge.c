...
typedef struct {
    ...
    uint8_t *thunk;       /* r-x: MOVZ W16,#idx ; jump entry        */
...
/* ------------------------------------------------------------------ */
/* Hook entry: thunk(W16=idx) -> naked -> body                          */
/* ------------------------------------------------------------------ */
...
{
    /* The thunk leaves the hook index in W17.  Spill it to a fixed slot
     * in this frame so the C body can pick it up via its frame pointer —
     * thread-safe (no globals) and immune to prologue clobbering:
     *   [entry_fp+0/+8]  saved x29/x30
     *   [entry_fp+16]    idx slot  (== body_fp + 32)
     */
    __asm__ volatile(
        "mov x9, x17\n"
        "stp x29, x30, [sp, #-32]!\n"
...
int64_t green_java_hook_body(JNIEnv *env, jobject thiz_or_cls,
                             ...
                             int64_t a4, int64_t a5, int64_t a6)
{
    int idx = *(const int *)((const uint8_t *)__builtin_frame_address(0)
                             + 32);
    GreenJavaHook *h;
    GreenJCtx *c;
    ...
    c = &g_jctx[g_jctx_sp++];
    c->env = env;
    /* The receiver arrives as a JNI local ref, which may be invalidated
     * (or have its table overflow) while the JS callback runs — pin it
     * with a global ref and drop it on the way out. */
    c->recv = h->is_static ? NULL
        : (*env)->NewGlobalRef(env, thiz_or_cls);
    c->cls = h->cls;
    c->backup = h->backup;
    c->ret = h->ret;
    c->hook_idx = idx;
    ...
    if (c->recv != NULL)
        (*env)->DeleteGlobalRef(env, c->recv);
    g_jctx_sp--;
    return rv;
}
                                   ...
                                   int argc, JSValueConst *argv)
{
    ...
    GreenJavaHook *h;
    JNIEnv *env;
    int64_t qargs[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
    int i, n;
    int64_t raw;
    ...
    n = h->nargs;
    qargs[0] = (int64_t)(intptr_t)c->backup;     /* x0 = ArtMethod (quick) */
    qargs[1] = (int64_t)(intptr_t)green_jobj_raw(c->recv); /* x1 raw recv */
    for (i = 0; i < n && i < 5; i++) {
        ...
        if (t == 'L' || t == '[') {
            ...
            qargs[i + 2] = (int64_t)(intptr_t)green_jobj_raw(hnd);
        } else {
            int64_t v = 0;
            JS_ToInt64(ctx, &v, el);
            qargs[i + 2] = v;
    ...
    LOGI("backup quick call: fn=%p x1(recv)=%llx x7(backup)=%llx nargs=%d",
         h->orig_quick, (unsigned long long)qargs[1],
         (unsigned long long)qargs[7], n);
    raw = green_call_asm(h->orig_quick, qargs, 8);
    LOGI("backup quick returned: %lld", (long long)raw);

    /* Marshal the raw x0 back per the return type. */
    switch (c->ret) {
    case 'V': return JS_UNDEFINED;
    case 'L': case '[': {
        jobject o = (jobject)(intptr_t)raw;
        if (o == NULL)
            return JS_NULL;
        /* The quick call yields a raw pointer; registering it back as a
         * global handle keeps every later JNI use working. */
        {
            jobject g = (*env)->NewGlobalRef(env, o);
            JSValue r = g ? green_jobj_wrap(env, g) : JS_NULL;
            return r;
        }
    }
    case 'J': return JS_NewBigInt64(ctx, raw);
    case 'D': case 'F': {
        union { int64_t i; double d; } cv;
        cv.i = raw;
        return JS_NewFloat64(ctx, cv.d);
    }
    default: return JS_NewInt64(ctx, raw);
    }
}
