...
The first version supports scalar GPR memory instructions:
...
uses only the read callback, so a write to a shadow code page still follows the
normal permission-fault path.
